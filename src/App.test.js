...
function App() {
  ...
  const [expanded, setExpanded] = useState(Array(molecules.length).fill(false));
            ...
            <React.Fragment key={index}>
              ...
              {expanded[index] && (
                <tr className="expanded-row">
                  <td colSpan="5">
                    <div>
                      {mol.notes && <p><strong>Notes:</strong> {mol.notes}</p>}
                      {mol.reference_electrode && <p><strong>Reference Electrode:</strong> {mol.reference_electrode}</p>}
                      {mol.measurement_methods && <p><strong>Measurement Method:</strong> {mol.measurement_methods.join(", ")}</p>}
                      <p><strong>Electrochemical Detection:</strong> {mol.electrochemical_detection ? "True" : "False"}</p>
                      {mol.indicator_electrode && <p><strong>Indicator Electrode:</strong> {mol.indicator_electrode}</p>}
                      {mol.counter_electrode && <p><strong>Counter Electrode:</strong> {mol.counter_electrode}</p>}
                      {mol.detection_antibodies && <p><strong>Detection Antibodies:</strong> {mol.detection_antibodies}</p>}
                      {mol.source && (
                        <p><strong>Source:</strong> <a href={mol.source} target="_blank" rel="noopener noreferrer">{mol.source}</a></p>
                      )}
                    </div>
                  </td>
                </tr>
